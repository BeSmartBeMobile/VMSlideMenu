...
class SectionCollectionViewCell: UICollectionViewCell {
    ...
    var summary: SectionCollectionViewCellSummary? {
        didSet {
            guard let options = summary?.options else {
                return
            }
...
extension SectionCollectionViewCell {
    ...
    func initializeView() {
        ...
        // Constraints
        let views: [String : Any] = ["self": self,
                                     "scrollView": scrollView]

        addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|[scrollView(==self)]|",
                                                      options: [],
                                                      metrics: nil,
                                                      views: views))

        addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[scrollView(==self)]|",
                                                      options: [],
                                                      metrics: nil,
                                                      views: views))
    }
    ...
    func reloadView() {
        ...
        var views: [String : Any] = ["self": self,
                                     "scrollView": scrollView]

        let metrics = ["mainHeight": mainHeight,
    ...
    func optionTouchUpInside(_ gestureRecognizer: UITapGestureRecognizer) {
        ...
        guard let optionView = gestureRecognizer.view as? OptionView,
            let index = optionViews.index(of: optionView) else {
                return
        }
